...
export default function ShareStory({ team, formation, positions, lineup, matchLabel, matchDateLabel }) {
  ...
  return (
    <div style={{ width: 1080, height: 1920 }} className="relative overflow-hidden bg-[#03123d] text-white">
      ...
      <div className="relative z-10 mx-auto mt-4 flex justify-center">
        <div style={{ width: pitchWidth }}>
          <div className="relative aspect-[10/14] overflow-hidden rounded-[38px] border-4 border-white/85 bg-emerald-700 shadow-[0_24px_44px_rgba(2,6,23,0.34),inset_0_12px_36px_rgba(255,255,255,0.08)]">
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,0.07)_50%,transparent_50%)] bg-[length:82px_82px]" />
            <div className="absolute left-0 right-0 top-1/2 h-1 -translate-y-1/2 bg-white/85" />
            <div className="absolute left-1/2 top-1/2 h-[162px] w-[162px] -translate-x-1/2 -translate-y-1/2 rounded-full border-4 border-white/85" />
            <div className="absolute left-1/2 top-0 h-[140px] w-[300px] -translate-x-1/2 rounded-b-[36px] border-x-4 border-b-4 border-white/85" />
            <div className="absolute bottom-0 left-1/2 h-[140px] w-[300px] -translate-x-1/2 rounded-t-[36px] border-x-4 border-t-4 border-white/85" />
            ...
            {positions.map((position) => {
              ...
              return (
                <div
                  key={position.id}
                  className="absolute z-10 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-1 rounded-[20px] border-[2px] border-yellow-400 bg-[#04103b] px-3 py-3 text-center shadow-[0_16px_28px_rgba(2,6,23,0.35)]"
                  style={{ left: `${position.x}%`, top: `${position.y}%`, minWidth: 132 }}
                >
                  <div className="flex h-[56px] w-[56px] items-center justify-center rounded-full bg-yellow-400 text-[22px] font-black text-slate-950">
                    {player ? initials(player) : position.label}
                  </div>
                  <div className="max-w-[118px] truncate text-[15px] font-black text-white">{player || position.label}</div>
                </div>
              );
            })}
          </div>

...
const rankings = {
  ...
  positions: {
    GOL: [['Alisson',1432], ['Ederson',881], ['Weverton',277]],
    LE: [['Alex Sandro',904], ['Douglas Santos',744]],
    ZAG: [['Marquinhos',1510], ['Gabriel Magalhães',1324], ['Bremer',993]],
    LD: [['Danilo',1138], ['Wesley',690]],
    VOL: [['Casemiro',1211], ['Bruno Guimarães',1048], ['Fabinho',812]],
    MC: [['Bruno Guimarães',1295], ['Lucas Paquetá',1167], ['Danilo S.',508]],
    PE: [['Vini Jr.',1710], ['Gabriel Martinelli',887]],
    ATA: [['Neymar Jr.',1520], ['Endrick',1403], ['Matheus Cunha',920]],
    PD: [['Raphinha',1299], ['Luiz Henrique',808], ['Rayan',477]],
  },
};
...
async function shareOrCopy(text) {
  try {
    if (navigator.share) {
      await navigator.share({ title: 'Escale sua Seleção', text, url: window.location.href });
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <Header page={page} setPage={setPage} user={user} setUser={setUser} openAuth={() => setAuthOpen(true)} count={selectedPlayers.length} formation={formation} />
      {page === 'escale' ? (
        <main className="mx-auto grid max-w-7xl gap-6 px-4 py-6 md:grid-cols-[370px_1fr] md:px-8">
          <aside className="space-y-5">
            <Panel title="Formação">
              <div className="grid grid-cols-2 gap-2">{Object.keys(formations).map((item) => <button key={item} onClick={() => resetLineup(item)} className={`rounded-2xl px-4 py-3 text-sm font-black transition ${formation === item ? 'bg-yellow-400 text-slate-950' : 'bg-slate-900 text-slate-200 hover:bg-slate-800'}`}>{item}</button>)}</div>
            </Panel>
            <Panel title="Jogadores" subtitle={selectedPlayer ? `Selecionado: ${selectedPlayer}` : 'Clique em um jogador para escalar.'} action={<button onClick={() => resetLineup()} className="rounded-xl bg-slate-900 p-2 text-slate-200 hover:bg-slate-800"><RotateCcw size={18} /></button>}>
              <div className="max-h-[620px] space-y-4 overflow-auto pr-1">
                {availablePlayers.map(({ category, names }) => <div key={category}><h3 className="mb-2 text-xs font-black uppercase tracking-[0.2em] text-yellow-300">{category}</h3><div className="grid grid-cols-2 gap-2">{names.map((name) => <button key={name} onClick={() => setSelectedPlayer(name)} className={`rounded-2xl border px-3 py-2 text-left text-sm font-bold transition ${selectedPlayer === name ? 'border-yellow-300 bg-yellow-300 text-slate-950' : 'border-white/10 bg-slate-900 text-slate-100 hover:border-yellow-300/60 hover:bg-slate-800'}`}>{name}<span className="mt-1 block text-[10px] font-medium opacity-70">{categoryByPlayer[name]}</span></button>)}</div></div>)}
              </div>
            </Panel>
          </aside>
          ...
          <LineupPanel positions={positions} lineup={lineup} complete={complete} saved={saved} shareStatus={shareStatus} saveLineup={saveLineup} shareText={shareText} setShareStatus={setShareStatus} />
        </main>
      ) : <RankingPage logged={Boolean(user)} openAuth={() => setAuthOpen(true)} setPage={setPage} />}
      {authOpen && <AuthModal onClose={() => setAuthOpen(false)} onAuth={(nextUser) => { setUser(nextUser); setAuthOpen(false); }} />}
    </div>
  );
}

function Header({ page, setPage, user, setUser, openAuth, count, formation }) {
  return <header className="border-b border-white/10 bg-slate-950/90 px-4 py-5 backdrop-blur md:px-8"><div className="mx-auto flex max-w-7xl flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full bg-yellow-400 px-3 py-1 text-xs font-black uppercase tracking-[0.25em] text-slate-950"><Trophy size={14} /> Seleção Oficial</div><h1 className="text-3xl font-black tracking-tight md:text-5xl">Escale sua Seleção</h1><p className="mt-2 max-w-2xl text-sm text-slate-300 md:text-base">Monte sua seleção, salve seu voto e compare com os jogadores mais escalados do Brasil.</p></div><div className="flex flex-col gap-3 md:items-end"><div className="flex flex-wrap gap-2"><button onClick={() => setPage('escale')} className={`rounded-2xl px-4 py-2 text-sm font-black transition ${page === 'escale' ? 'bg-yellow-400 text-slate-950' : 'bg-white/10 text-white hover:bg-white/15'}`}>Escalar</button><button onClick={() => setPage('ranking')} className={`rounded-2xl px-4 py-2 text-sm font-black transition ${page === 'ranking' ? 'bg-yellow-400 text-slate-950' : 'bg-white/10 text-white hover:bg-white/15'}`}>Mais escalados</button>{user ? <button onClick={() => setUser(null)} className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-4 py-2 text-sm font-black text-slate-200 hover:bg-slate-800"><LogOut size={16} /> Sair de {user.name.split(' ')[0]}</button> : <button onClick={openAuth} className="inline-flex items-center gap-2 rounded-2xl bg-emerald-500 px-4 py-2 text-sm font-black text-slate-950 hover:bg-emerald-400"><LogIn size={16} /> Entrar</button>}</div><div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200"><div className="flex items-center gap-2 font-bold"><Users size={18} /> {count}/11 escalados</div><div className="mt-1 text-xs text-slate-400">Formação atual: <strong className="text-yellow-300">{formation}</strong></div></div></div></div></header>;
}
...
function Field({ positions, lineup, selectedPlayer, formation, clickPosition, removePlayer }) {
  return <section className="rounded-[2rem] border border-white/10 bg-white/5 p-3 shadow-2xl md:p-5 md:col-start-2 md:row-span-2"><div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><div><h2 className="text-2xl font-black">Campo</h2><p className="text-sm text-slate-400">Monte seu time ideal no esquema {formation}.</p></div><div className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-4 py-2 text-xs font-bold text-slate-300"><MousePointerClick size={16} /> Clique na posição para colocar o jogador</div></div><div className="relative mx-auto aspect-[10/14] max-h-[760px] overflow-hidden rounded-[2rem] border-4 border-white/80 bg-emerald-700 shadow-inner"><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,0.05)_50%,transparent_50%)] bg-[length:80px_80px]" /><div className="absolute left-0 right-0 top-1/2 h-1 -translate-y-1/2 bg-white/80" /><div className="absolute left-1/2 top-1/2 h-32 w-32 -translate-x-1/2 -translate-y-1/2 rounded-full border-4 border-white/80" /><div className="absolute left-1/2 top-0 h-28 w-56 -translate-x-1/2 rounded-b-3xl border-x-4 border-b-4 border-white/80" /><div className="absolute bottom-0 left-1/2 h-28 w-56 -translate-x-1/2 rounded-t-3xl border-x-4 border-t-4 border-white/80" />{positions.map((position) => { const player = lineup[position.id]; return <button key={`${formation}-${position.id}`} onClick={() => player ? removePlayer(position.id) : clickPosition(position.id)} className={`absolute z-10 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-1 rounded-2xl border px-2 py-2 text-center shadow-xl transition-colors md:min-w-28 md:px-3 ${player ? 'border-yellow-300 bg-slate-950 text-white' : selectedPlayer ? 'border-yellow-300 bg-yellow-300 text-slate-950 hover:bg-yellow-200' : 'border-white/40 bg-white/15 text-white hover:bg-white/25'}`} style={{ left: `${position.x}%`, top: `${position.y}%` }}><span className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-black md:h-12 md:w-12 ${player ? 'bg-yellow-400 text-slate-950' : 'bg-slate-950/80 text-white'}`}>{player ? initials(player) : position.label}</span><span className="max-w-24 truncate text-xs font-black md:max-w-28 md:text-sm">{player || position.label}</span></button>; })}</div></section>;
}

function LineupPanel({ positions, lineup, complete, saved, shareStatus, saveLineup, shareText, setShareStatus }) {
  return <section className="rounded-3xl border border-white/10 bg-slate-950 p-4 md:col-start-2"><div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h3 className="text-lg font-black">Minha escalação</h3><p className="text-xs text-slate-400">{saved ? 'Escalação salva. Seu voto entrou no ranking.' : 'Complete os 11 jogadores para salvar e compartilhar.'}</p></div><div className="flex flex-wrap gap-2"><button onClick={saveLineup} disabled={!complete} className="inline-flex items-center gap-2 rounded-2xl bg-yellow-400 px-4 py-2 text-sm font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-40"><Save size={16} /> Salvar voto</button><button onClick={async () => setShareStatus(await shareOrCopy(shareText))} disabled={!complete} className="inline-flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-2 text-sm font-black text-white hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-40"><Share2 size={16} /> Compartilhar</button></div></div><div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">{positions.map((position) => <div key={position.id} className="rounded-2xl bg-white/5 px-3 py-2 text-sm"><span className="font-black text-yellow-300">{position.label}</span><span className="mx-2 text-slate-500">•</span><span className="font-bold text-slate-100">{lineup[position.id] || 'Em aberto'}</span></div>)}</div><div className="mt-4 grid gap-2 md:grid-cols-3"><button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(shareText)}`, '_blank', 'noopener,noreferrer')} disabled={!complete} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 py-3 text-sm font-black text-slate-950 hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-40"><MessageCircle size={18} /> WhatsApp</button><button onClick={async () => setShareStatus((await copyText(shareText)) ? 'Texto copiado para colar no Instagram.' : shareText)} disabled={!complete} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-pink-500 px-4 py-3 text-sm font-black text-white hover:bg-pink-400 disabled:cursor-not-allowed disabled:opacity-40"><Share2 size={18} /> Instagram</button><button onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}&quote=${encodeURIComponent(shareText)}`, '_blank', 'noopener,noreferrer')} disabled={!complete} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-500 px-4 py-3 text-sm font-black text-white hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-40"><Share2 size={18} /> Facebook</button></div>{shareStatus && <div className="mt-4 whitespace-pre-wrap rounded-2xl border border-yellow-300/30 bg-yellow-300/10 p-3 text-xs font-bold text-yellow-100">{shareStatus}</div>}</section>;
}

...
async function shareOrCopy(text) {
  try { if (navigator.share) { await navigator.share({ title: 'Escale sua Seleção', text, url: window.location.href }); return 'Compartilhamento aberto.'; } } catch {}
...
export default function App() {
  ...
  function clickPosition(positionId) {
    ...
    setSelectedPlayer(null); setSaved(false); setStatus('');
  }

  function removePlayer(positionId) {
    setLineup((current) => { const copy = { ...current }; delete copy[positionId]; return copy; });
    setSaved(false); setStatus('');
  }
...
function LineupPanel({ positions, lineup, complete, saved, status, saveLineup, shareText, setStatus }) {
...
function AuthModal({ onClose }) {
  ...
  async function submit(event) {
    event.preventDefault(); setMessage(register ? 'Criando cadastro...' : 'Entrando...');
    ...
    else setMessage(register ? 'Cadastro criado. Verifique seu e-mail se a confirmação estiver ativa.' : 'Login realizado.');
  }
